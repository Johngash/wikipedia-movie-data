...
let years = [];
for (let year = 2025; year <= 2026; year++) years.push(year);
...
async function main() {
  let movies = [];
  // const decadeFile = "movies-2020s.json";
  // if (fs.existsSync(decadeFile)) {
  //   movies = JSON.parse(fs.readFileSync(decadeFile));
  // }
  let newMovies = [];
  for (let year of years) {
    const scrapedMovies = await scrapeMoviesForYear(year);
    newMovies = newMovies.concat(scrapedMovies);
    movies = movies.concat(scrapedMovies);
  }

  for (let movie of newMovies) {
    await timeout(50);
    let res = await request({
      url: `https://en.wikipedia.org/api/rest_v1/page/summary/${movie.href}`,
      headers: {
        "User-Agent":
    ...
    if (res.statusCode != 200) {
      throw new Error(
        "wikipedia returned an error response: " + res.statusCode,
      );
    }
  ...
  fs.writeFileSync(decadeFile, JSON.stringify(movies, null, 2), {
    encoding: "utf8",
  });
}
...
async function scrapeMoviesForYear(year) {
  ...
  if (!tables.length) {
    console.log(body);
    throw new Error(
      'Did not find a table w/ class "wikitable" in Wikipedia\'s response',
    );
  }
  ...
  tables.each(function (ix, table) {
    ...
    if ($(rows[0]).text().toLowerCase().includes("gross")) return;
    ...
    rows.each(function (ix, el) {
      ...
      if (isDateCell(title_cell)) title_cell = $(cells[1]);
      if (isDateCell(title_cell)) title_cell = $(cells[2]);
      ...
      if (!title_cell.text().trim()) return;
      ...
      if (href && href.includes("action=edit")) {
        ...
        // these are normal links, clean them up
        assert(
          href.includes("/wiki/"),
          `Expected "${href}" to include "/wiki/"`,
        );
      ...
      console.log(
        `${movie_data.year} ${movie_data.genres.join(",")} (${movie_data.cast.join(", ")}) "${movie_data.title}"`,
      );
...
function cleanGenres(genres, year) {
  ...
  genres.forEach(function (genre) {
    ...
    if (whitelist[genre]) cleaned_genres.push(whitelist[genre]);
...
function toArray(cell) {
  ...
  cell.contents().each(function (ix, el) {
    ...
    text_parts.forEach(function (text, ix) {
      ...
      if (text == "Jr.") arr[arr.length - 1] += ", Jr.";
      else if (text == "-") return;
